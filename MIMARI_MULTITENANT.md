# DesaDoor — Multi-Tenant Mimari Planı
...
4. Bulamazsa varsayılan firmayı (DesaDoor) ata
5. Geliştirme ortamında `?firma=slug` query param ile override

**Dosya:** `Desadoor.Api/AraYazilimlar/FirmaCozumlemeMiddleware.cs`
...
**Dosya:** `Desadoor.Api/Servisler/KiraciServisi.cs`
...
Public sayfalar (`DesaDoorDuzen.razor`) firma bilgilerini API'den çeker:

```csharp
// DesaDoorDuzen.razor.cs OnInitializedAsync
...
```
Geliştirme:
  localhost:5013?firma=desadoor → DesaDoor
  localhost:5013?firma=firma2   → Firma 2

Üretim:
  desadoor.com.tr               → DesaDoor (Domain eşleşmesi)
