...
public partial class Urunler : ComponentBase, IDisposable
{
    ...
    /// <summary>
    /// Görüntüleme koleksiyon anahtarı: statik Gold Banyo katalogunda varsa
    ...
    private IEnumerable<string> KoleksiyonAdlari =>
        TumUrunler.Select(KoleksiyonAnahtari).Distinct()
            .OrderBy(GoldBanyoKatalogUrunleri.KoleksiyonSirasiBul)
            .ThenBy(ad => ad);
    ...
    private IEnumerable<KoleksiyonGrubu> Gruplar =>
        TumUrunler
            ...
            .OrderBy(g => GoldBanyoKatalogUrunleri.KoleksiyonSirasiBul(g.Key))
            .ThenBy(g => g.Key)
            .Select(g =>
            {
                var aciklama = GoldBanyoKatalogUrunleri.KoleksiyonAciklamasiBul(g.Key);
