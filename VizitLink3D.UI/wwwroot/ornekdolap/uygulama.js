...
const modelYolu = "./model/FUGA_2_DOLAP.glb";
...
function modelParcasiniSiniflandir(mesh, modelKutu) {
  ...
  // Mesh adlarına göre doğrudan eşleme (yeni sistem)
  // Hem space hem underscore pattern'lerini kontrol et
  if (ad.includes("gövde") || ad.includes("govde")) return "govde";
  if (ad.includes("ayna") || ad.includes("atna") || ad.includes("mirror") || ad.includes("glass") || ad.includes("gercek_ayna")) return "ayna";
  ...
  if (ad.includes("kulp")) return "kulp";
  if (ad.includes("led")) return "led";
  if (ad.includes("musluk") || ad.includes("faucet") || ad.includes("tap")) return "musluk";
  if (ad.includes("lavabo") || ad.includes("lavobo") || ad.includes("sink") || ad.includes("basin")) return "lavabo";
  if (ad.includes("üst tabla") || ad.includes("ust tabla") || ad.includes("ust_tabla")) return "ustTabla";
  if (ad.includes("iç üst tabla") || ad.includes("ic ust tabla") || ad.includes("ic_ust_tabla")) return "icUstTabla";
  if (ad.includes("iç alt tabla") || ad.includes("ic alt tabla") || ad.includes("ic_alt_tabla")) return "icAltTabla";
...
function modeliYukle() {
  if (typeof FUGA_MODEL_BASE64 !== "undefined" && FUGA_MODEL_BASE64) {
    console.log("📦 Gömülü model verisi kullanılıyor (base64, sunucu gerekmez)");
    const arrayBuffer = base64ArrayBufferineCevir(FUGA_MODEL_BASE64);
    modeliYukleArrayBufferden(arrayBuffer, "FUGA DOLAP.glb");
    return;
  }

  if (window.location.protocol === "file:") {
    console.warn("⚠️ File protocol — gömülü model verisi bulunamadı");
    yuklemeKatmaniniAyarla(false);
    return;
  }

  modeliYukleAdresten(modelYolu, "FUGA DOLAP.glb");
}
