...
public class LisansKontrolcu(DesadoorDbContext vt, KiraciServisi kiraci, LisansServisi lisansServisi, IConfiguration konfigurasyon) : ControllerBase
{
    [HttpGet("durum")]
    ...
    {
        var gizliAnahtar = Environment.GetEnvironmentVariable("DESADOOR_LISANS_KEY")
    ...
    string Mesaj)
{
    public static LisansDurumDto Olustur(Firma firma, Lisans? lisans, LisansDurumu durum)
        => new(
            ...
            durum.EkSuredeMi,
            string.IsNullOrWhiteSpace(durum.Sebep) ? "Lisans geçerli." : durum.Sebep);
}
