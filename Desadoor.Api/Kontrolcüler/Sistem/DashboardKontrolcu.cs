...
[ApiController]
[Route("api/dashboard")]
public class DashboardKontrolcu(DesadoorDbContext vt) : ControllerBase
{
    ...
    public async Task<Cevap<DashboardKomutaMerkezi>> KomutaMerkeziGetir()
    {
        ...
        var komuta = new DashboardKomutaMerkezi
        {
            ...
            {
                Api = "Çevrimiçi",
                Veritabani = "Çevrimiçi",
                CanliBaglanti = "SignalR hazır",
                Lisans = "Aktif",
                SonGuncelleme = simdi
            }
        };
    ...
    private static string RefererKaynak(string? referer)
    {
        ...
        return Uri.TryCreate(referer, UriKind.Absolute, out var uri) ? uri.Host.Replace("www.", "") : "Site içi";
    }
