...
[ApiController]
[Route("api/dil")]
public class DilKontrolcu(DesadoorDbContext vt, ICeviriServisi ceviriServisi) : ControllerBase
{
    /// <summary>
    ...
    public async Task<IActionResult> CeviriKaydet([FromBody] Ceviri ceviri)
    {
        ...
        return Ok(Cevap<Ceviri>.Basarili(mevcut ?? ceviri, "Çeviri kaydedildi."));
    }
    ...
    public async Task<IActionResult> AICeviriEkle([FromBody] AICeviriEkleIstegi istek)
    {
        if (string.IsNullOrWhiteSpace(istek.Anahtar) || string.IsNullOrWhiteSpace(istek.Deger))
            return Ok(Cevap<bool>.Hata("Anahtar veya değer boş."));
        ...
        return Ok(Cevap<bool>.Basarili(true, "AI çeviri kaydedildi."));
    }
    ...
    public async Task<IActionResult> DilGuncelle(int id, [FromBody] Dil dil)
    {
        ...
        if (mevcut is null) return Ok(Cevap<Dil>.Hata("Dil bulunamadı."));

        mevcut.AktifMi = dil.AktifMi;
        mevcut.SiraNo = dil.SiraNo;
        await vt.SaveChangesAsync();
        await ceviriServisi.OnbellekTemizleAsync();
        return Ok(Cevap<Dil>.Basarili(mevcut, "Dil güncellendi."));
    }
