...
namespace Desadoor.Api.Moduller.Medya.Kontrolcu;
        ...
        var belge = Document.Create(konteyner =>
        {
            konteyner.Page(sayfa =>
            {
                ...
                sayfa.Header()
                    .Row(row =>
                    {
                        row.RelativeItem().Text("DesaDoor").Bold().FontSize(20);
                ...
                sayfa.Footer()
                    .AlignCenter()
                    .Text("DesaDoor © 2024 — Bursa Çalı | www.desadoor.com.tr").FontSize(8);
            });
        });

        var pdf = belge.GeneratePdf();
        return File(pdf, "application/pdf", $"DesaDoor_Teklif_{istek.ModelKodu}.pdf");
    }
