...
public class KapakGocServisi(DesadoorDbContext vt)
{
    public async Task<Cevap<GocSonucu>> GogAsync(CancellationToken iptal = default)
    {
        ...
        foreach (var km in kapakModelleri)
        {
            ...
            if (!string.IsNullOrWhiteSpace(km.AnaGorselUrl))
            {
                ...
                var anaMedya = new Desadoor.Ortak.Modeller.Medya.Medya
                {
                    ...
                    Tip = Desadoor.Ortak.Modeller.Medya.MedyaTipi.Resim,
                    Kaynak = Desadoor.Ortak.Modeller.Medya.MedyaKaynagi.Yerel,
                    OlusturulmaTarihi = DateTime.UtcNow
                };
