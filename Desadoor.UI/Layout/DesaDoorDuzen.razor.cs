...
namespace Desadoor.UI.Layout;

/// <summary>
/// DesaDoor kurumsal web sitesinin ana layout mantık dosyasıdır.
...
/// </summary>
public partial class DesaDoorDuzen : IAsyncDisposable
{
    ...
    /// </summary>
    private string _logoUrl = "/img/desadoor-logo-light.svg";
    ...
    private string _footerAciklama = "Bursa Çalı bölgesinde kurulan üretim tesisimizde akrilik, laminat, UV lak, membran, lake kapı ve ev/ofis mobilyaları üretilmektedir.";
    private string _facebookUrl = "https://facebook.com/desadoor";
    private string _instagramUrl = "https://instagram.com/desadoor.com.tr";
    private string _youtubeUrl = "https://youtube.com/desadoor";
    private string _pinterestUrl = "https://pinterest.com/desadoor";
    ...
    private string _mesaiSaatleri = "09:00 – 18:00";
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            var kayitliDil = await js.InvokeAsync<string>("localStorage.getItem", "desadoordil");
            ...
            {
                uygulanacakTema = await js.InvokeAsync<string>("localStorage.getItem", "desadoor_site_tema");
            }

            if (!string.IsNullOrWhiteSpace(uygulanacakTema))
            {
                await js.InvokeVoidAsync("desadoorTema.siteUygula", uygulanacakTema);
            }
        }
        catch { }
    ...
    private async Task AyarlariYukle()
    {
        try
        {
            var sozluk = await api.GetAsync<Dictionary<string, string>>($"api/desadoor/sayfa-icerigi/ayarlar?dil={dil.AktifDil}");
            if (sozluk != null)
            {
    ...
    private async Task MenuOgeleriniYukle()
    {
        ...
        try
        {
            ...
            var liste = await api.GetAsync<List<MenuOgesi>>("api/menu/desadoor");
            if (liste?.Count > 0)
            {
    ...
    /// Bu liste desadoor.com.tr mevcut site yapısını yansıtır.
    /// </summary>
    private List<MenuOgesi> VarsayilanMenuOlustur() => new()
    {
        ...
        new MenuOgesi { Baslik = dil.T("kapak_sistemleri", "Kapak Sistemleri"), Url = "kapak-sistemleri", AltMenuler = new() {
            new MenuOgesi { Baslik = "Akrilik Kapak", Url = "akrilik" },
            new MenuOgesi { Baslik = "Laminat Kapak", Url = "laminat" },
            new MenuOgesi { Baslik = "Membran Kapak", Url = "membran" },
            new MenuOgesi { Baslik = "Lake Kapak", Url = "lake" }
        } },
        new MenuOgesi { Baslik = dil.T("kapi_modelleri", "Kapı Modelleri"),      Url = "kapi-modelleri" },
    ...
    private async Task DilDegistir(string yeniDil)
    {
        _aktifDil = yeniDil;
        _dilMenusuAcik = false;
        await js.InvokeVoidAsync("localStorage.setItem", "desadoordil", yeniDil);
