...
public partial class SeoYonetimi : ComponentBase
{
    ...
    private async Task SayfalariYukleAsync()
    {
        ...
        try
        {
            var icerikler = await Api.GetAsync<List<SayfaIcerigiDto>>("api/desadoor/sayfa-icerigi");
    ...
    private async Task SeciliSayfaDegisti(string bolum)
    {
        ...
        try
        {
            var yanit = await Api.GetAsync<Dictionary<string, string>>($"api/desadoor/sayfa-icerigi/{bolum}");
            if (yanit != null)
            {
