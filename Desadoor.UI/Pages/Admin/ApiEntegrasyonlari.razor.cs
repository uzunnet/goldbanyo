...
public partial class ApiEntegrasyonlari : ComponentBase
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try 
        {
            var ayarlar = await api.GetAsync<List<SistemAyari>>("api/desadoor/ayarlar/api-entegrasyon");
            if (ayarlar != null)
            {
    ...
    private async Task KaydetAnalytics()
    {
        ...
        var sonuc = await api.PostAsync<object>("api/desadoor/ayarlar/analytics", analitikAyarlar);
    ...
    private async Task KaydetAi()
    {
        ...
        var sonuc = await api.PostAsync<object>("api/desadoor/ayarlar/ai", aiAyarlar);
    ...
    {
        var sonuc = await api.GetAsync<bool>("api/desadoor/ayarlar/ai/test");
        if (sonuc)
