...
public partial class HaberYonetimi : ComponentBase
{
    ...
    async Task Yukle()
    {
        ...
        _liste = await Api.GetAsync<List<HaberYazisi>>("api/desadoor/Haber-yazilari") ?? [];
    ...
    {
        var cevap = await Api.DeleteAsync($"api/desadoor/Haber-yazilari/{x.Id}");
    ...
    async Task Kaydet()
    {
        ...
        if (_duzenlenenId.HasValue)
            cevap = await Api.PutAsync<HaberYazisi>($"api/desadoor/Haber-yazilari/{_duzenlenenId.Value}", _form);
        else
            cevap = await Api.PostAsync<HaberYazisi>("api/desadoor/Haber-yazilari", _form);
