...
public partial class KullaniciYonetimi : ComponentBase
{
    ...
    async Task Yukle()
    {
        _yukleniyor = true;
        StateHasChanged();
        _liste = await Api.GetAsync<List<Kullanici>>("api/desadoor/kullanicilar") ?? [];
        AramaUygula();
        _yukleniyor = false;
    }
    ...
    {
        await Api.DeleteAsync($"api/desadoor/kullanicilar/{k.Id}");
    ...
    async Task Kaydet()
    {
        ...
        try
        {
            if (_duzenlenenId.HasValue)
                await Api.PutAsync<Kullanici>($"api/desadoor/kullanicilar/{_duzenlenenId.Value}", _form);
            else
                await Api.PostAsync<Kullanici>("api/desadoor/kullanicilar", _form);
