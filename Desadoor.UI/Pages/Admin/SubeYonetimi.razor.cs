...
public partial class SubeYonetimi : ComponentBase
{
    ...
    async Task Yukle()
    {
        _yukleniyor = true;
        StateHasChanged();
        _liste = await Api.GetAsync<List<Sube>>("api/desadoor/subeler") ?? [];
    ...
    {
        var cevap = await Api.DeleteAsync($"api/desadoor/subeler/{s.Id}");
    ...
    async Task Kaydet()
    {
        ...
        if (_duzenlenenId.HasValue)
            cevap = await Api.PutAsync<Sube>($"api/desadoor/subeler/{_duzenlenenId.Value}", _form);
        else
            cevap = await Api.PostAsync<Sube>("api/desadoor/subeler", _form);
