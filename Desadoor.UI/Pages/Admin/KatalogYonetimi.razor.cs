...
public partial class KatalogYonetimi : ComponentBase
{
    ...
    async Task Yukle()
    {
        _yukleniyor = true;
        StateHasChanged();
        _liste = await Api.GetAsync<List<Katalog>>("api/desadoor/kataloglar") ?? [];
    ...
    async Task DosyaYukle(IBrowserFile dosya)
    {
        ...
        var cevap = await Api.PostMultipartAsync<KatalogDosyaYuklemeSonucu>("api/desadoor/kataloglar/dosya-yukle", icerik);
    ...
    {
        var cevap = await Api.DeleteAsync($"api/desadoor/kataloglar/{k.Id}");
    ...
    async Task Kaydet()
    {
        ...
        if (_duzenlenenId.HasValue)
            cevap = await Api.PutAsync<Katalog>($"api/desadoor/kataloglar/{_duzenlenenId.Value}", _form);
        else
            cevap = await Api.PostAsync<Katalog>("api/desadoor/kataloglar", _form);
    ...
    async Task AICeviriDialogAc(Katalog k)
    {
        ...
        var dialog = await DialogServisi.ShowAsync<Desadoor.UI.Bilesenler.Admin.AICeviriDialog>("🌍 Yapay Zeka Çevirisi", parameters);
        await dialog.Result;
        await Yukle();
    }
