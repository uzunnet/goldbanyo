...
public partial class MedyaGalerisi : ComponentBase
{
    ...
    {
        _yukleniyor = true;
        _gorseller = await Api.GetAsync<List<GaleriGorseliDto>>("api/desadoor/galeri-gorselleri/detay") ?? [];
    ...
    private async Task YuklemeyiBaslat()
    {
        ...
        foreach (var dosya in _yuklenecekDosyalar)
        {
            ...
            await Api.PostMultipartAsync<GaleriGorseliDto>("api/desadoor/galeri-gorselleri/yukle", icerik);
        }
    ...
    private async Task Kaydet()
    {
        ...
        var cevap = await Api.PutAsync<GaleriGorseliDto>(
            $"api/desadoor/galeri-gorselleri/{_duzenlenenGorsel.Id}",
            _duzenlenenGorsel);
        _kaydediliyor = false;
    ...
    {
        var cevap = await Api.DeleteAsync($"api/desadoor/galeri-gorselleri/{gorsel.Id}");
