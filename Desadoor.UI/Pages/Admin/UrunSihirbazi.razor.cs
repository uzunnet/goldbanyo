...
public partial class UrunSihirbazi : ComponentBase
{
    ...
    private async Task MedyaSec()
    {
        ...
        var d = await DialogServisi.ShowAsync<Desadoor.UI.Bilesenler.Medya.MedyaSecici>(dil.T("admin.medya.sec", "Medya seç"), p, opts);
        ...
        try
        {
            ...
            foreach (var kvp in cevrilecekMetinler)
            {
                ...
                if (ceviriYanit?.BasariliMi == true && !string.IsNullOrEmpty(ceviriYanit.Veri))
                {
                    ...
                    await Api.PostAsync<object>("api/yonetim/ceviri/kaydet", new Desadoor.Ortak.Modeller.Ceviriler.CeviriKayitIstegi 
                    { 
                        Anahtar = kvp.Key, 
