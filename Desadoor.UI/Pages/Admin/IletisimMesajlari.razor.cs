...
{
    [Inject] private Desadoor.UI.Servisler.AnimasyonMotoruServisi _animasyonMotoru { get; set; } = default!;
    ...
    {
        _yukleniyor = true;
        var yanit = await api.GetAsync<MesajListesiDto>("api/desadoor/iletisim/mesajlar");
        if (yanit != null)
        {
    ...
    private async Task MesajiAc(IletisimMesajiDto mesaj)
    {
        ...
        if (!mesaj.OkunduMu)
        {
            var yanit = await api.PatchAsync($"api/desadoor/iletisim/mesajlar/{mesaj.Id}/okundu");
    ...
    {
        var yanit = await api.DeleteAsync($"api/desadoor/iletisim/mesajlar/{mesaj.Id}");
        if (yanit?.BasariliMi == true)
        {
