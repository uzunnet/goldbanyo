...
public partial class EkipYonetimi : ComponentBase
{
    ...
    async Task Yukle()
    {
        _yukleniyor = true;
        StateHasChanged();
        _liste = await Api.GetAsync<List<EkipUyesi>>("api/desadoor/ekip") ?? [];
    ...
    async Task Kaydet()
    {
        ...
        if (_id.HasValue)
            cevap = await Api.PutAsync<EkipUyesi>($"api/desadoor/ekip/{_id.Value}", _form);
        else
            cevap = await Api.PostAsync<EkipUyesi>("api/desadoor/ekip", _form);
    ...
    {
        var cevap = await Api.DeleteAsync($"api/desadoor/ekip/{e.Id}");
        if (cevap?.BasariliMi == true)
        {
            Snackbar.Add(cevap.Mesaj, Severity.Success);
            await Yukle();
