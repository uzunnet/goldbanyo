...
public partial class SayfaDuzenle : ComponentBase
{
    ...
    {
        var yanit = await Api.GetAsync<Dictionary<string, string>>($"api/desadoor/sayfa-icerigi/{_sayfa.Bolum}");
    ...
    private async Task DosyaYukleAsync(IBrowserFile dosya)
    {
        if (dosya != null)
        {
            ...
            var sonuc = await Api.PostMultipartAsync<string>("api/desadoor/sayfa-icerigi/yukle/katalog", icerik);
