...
public partial class SertifikaYonetimi : ComponentBase
{
    ...
    private async Task Yukle()
    {
        _yukleniyor = true;
        StateHasChanged();
        _liste = await Api.GetAsync<List<Sertifika>>("api/desadoor/sertifikalar/yonetim") ?? [];
        AramaUygula();
        _yukleniyor = false;
    }
    ...
    private async Task DosyaYukle(IBrowserFile dosya)
    {
        ...
        var cevap = await Api.PostMultipartAsync<SertifikaDosyaYuklemeSonucu>("api/desadoor/sertifikalar/dosya-yukle", icerik);
    ...
    {
        var cevap = await Api.DeleteAsync($"api/desadoor/sertifikalar/{sertifika.Id}");
    ...
    private async Task Kaydet()
    {
        ...
        {
            cevap = await Api.PutAsync<Sertifika>($"api/desadoor/sertifikalar/{_duzenlenenId.Value}", _form);
        }
        else
        {
            cevap = await Api.PostAsync<Sertifika>("api/desadoor/sertifikalar", _form);
        }
