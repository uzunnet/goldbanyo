...
public partial class CanliSohbetArayuzu
{
    ...
    [Inject] public Desadoor.UI.Servisler.ApiIstemcisi Api { get; set; } = default!;
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try {
            var kaydedilmisId = await JSRuntime.InvokeAsync<string>("localStorage.getItem", "desadoor_sohbet_oturum");
            if (string.IsNullOrEmpty(kaydedilmisId)) {
                _oturumId = Guid.NewGuid().ToString().Substring(0, 8);
                await JSRuntime.InvokeVoidAsync("localStorage.setItem", "desadoor_sohbet_oturum", _oturumId);
            } else {
