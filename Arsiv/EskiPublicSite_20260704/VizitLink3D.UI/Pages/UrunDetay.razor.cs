...
public partial class UrunDetay : ComponentBase, IDisposable
{
    ...
    private GoldBanyoKatalogUrunu? _katalogUrunu;
    ...
    private async Task UrunDetayiniYukleAsync()
    {
        ...
        try
        {
            ...
            _katalogUrunu = GoldBanyoKatalogUrunleri.SlugIleGetir(Slug);
            _urun ??= KatalogtanYerelUrunOlustur(_katalogUrunu);
            if (_urun != null)
            {
                _katalogUrunu ??= GoldBanyoKatalogUrunleri.SlugIleGetir(_urun.Slug);
                _breadcrumb =
                [
                    new(dil.T("menu.anasayfa", "Ana Sayfa"), "/"),
                    new(dil.T("menu.urunler", "Banyo Dolapları"), "/banyo-dolaplari"),
                    new(_urun.Ad, null, disabled: true)
                ];

                if (_urun.Id > 0 && _urun.Id < 100000)
                {
                    await Task.WhenAll(
                        ModelYoluYukleAsync(),
                        RenkleriYukleAsync(),
                ...
                else if (_katalogUrunu != null)
                {
                    _medyalar =
                    [
                        new UrunMedya { MedyaUrl = _katalogUrunu.HeroGorselUrl, AnaGosterim = true, SiraNo = 1 },
                        new UrunMedya { MedyaUrl = _katalogUrunu.KatalogGorselUrl, AnaGosterim = false, SiraNo = 2 },
                        new UrunMedya { MedyaUrl = _katalogUrunu.TeknikGorselUrl, AnaGosterim = false, SiraNo = 3 }
                    ];
                }
    ...
    private static Urun? KatalogtanYerelUrunOlustur(GoldBanyoKatalogUrunu? katalogUrunu)
    {
        ...
        return new Urun
        {
            Id = 100000 + katalogUrunu.SayfaNo,
            Slug = katalogUrunu.Slug,
            Kod = katalogUrunu.Kod,
            Ad = katalogUrunu.Ad,
            KisaAciklama = katalogUrunu.KisaAciklamaOlustur(),
            Aciklama = katalogUrunu.AciklamaHtmlOlustur(),
            UrunAilesiId = 3,
            UrunKategoriId = 2,
            AktifMi = true,
            OneCikanMi = katalogUrunu.OneCikanMi,
            YeniMi = katalogUrunu.YeniMi,
            Fiyat = katalogUrunu.Fiyat,
            Birim = "adet",
            SiraNo = katalogUrunu.SayfaNo,
            SeoBaslik = $"{katalogUrunu.Ad} | Gold Banyo 2026 Katalog",
            SeoAciklama = katalogUrunu.KisaAciklamaOlustur()
        };
    }
    ...
    private void TeknikOzellikleriDoldur()
    {
        ...
        _teknikOzellikler = [];
        if (!string.IsNullOrEmpty(_urun.Kod)) _teknikOzellikler.Add(new(dil.T("urun.teknik_kod", "Ürün Kodu"), _urun.Kod));
        if (_katalogUrunu != null) _teknikOzellikler.Add(new(dil.T("urun.teknik_koleksiyon", "Koleksiyon"), _katalogUrunu.KoleksiyonGrubu));
        if (_urun.UrunAilesi != null) _teknikOzellikler.Add(new(dil.T("urun.teknik_aile", "Ürün Ailesi"), _urun.UrunAilesi.Ad));
        _teknikOzellikler.Add(new(dil.T("urun.teknik_ucboyut", "3D Görüntüleme"), _modelYolu != null ? dil.T("ortak.var", "Var") : dil.T("ortak.yok", "Yok")));
        if (_onayliParcalar.Count > 0) _teknikOzellikler.Add(new(dil.T("urun.teknik_parca", "Parça Sayısı"), _onayliParcalar.Count.ToString()));
        if (_renkler.Count > 0) _teknikOzellikler.Add(new(dil.T("urun.teknik_renk", "Renk Seçeneği"), $"{_renkler.Count} {dil.T("urun.renkSecenegi", "renk")}"));
        if (_katalogUrunu != null) _teknikOzellikler.Add(new(dil.T("urun.teknik_katalog", "Katalog Sayfası"), $"{_katalogUrunu.SayfaNo}-{_katalogUrunu.TumSayfalar.Last()}"));
        _teknikOzellikler.Add(new(dil.T("urun.teknik_durum", "Durum"), _urun.AktifMi ? dil.T("ortak.aktif", "Aktif") : dil.T("ortak.pasif", "Pasif")));
    }
