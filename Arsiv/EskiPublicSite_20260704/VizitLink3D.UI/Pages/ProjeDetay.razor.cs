...
public partial class ProjeDetay : ComponentBase
{
    ...
    private string _sayfaBasligi = "Proje Detayi | VizitLink3D";
    ...
    private async Task DetayYukleAsync()
    {
        ...
        if (_proje is not null)
            _sayfaBasligi = $"{_proje.Baslik} | VizitLink3D";

        _yukleniyor = false;
    }

    private static string ProjeGorseli(Proje proje)
        => string.IsNullOrWhiteSpace(proje.KapakResim) ? "/medya/vizitlink3d_default.png" : proje.KapakResim;
