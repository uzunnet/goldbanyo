...
public partial class Haber : ComponentBase, IDisposable
{
    ...
    private string _sayfaBasligi = "Haber | VizitLink3D";
    private string _heroBaslik = "Haber";
    private string _heroAciklama = "VizitLink3D'dan haberler ve icerikler.";
    ...
    private async Task HaberYazilariniYukleAsync()
    {
        ...
        try
        {
            var liste = await api.GetAsync<List<HaberOgesi>>($"api/Haber-yazilari?dil={dil.AktifDil}");
            if (liste != null)
                _yazilar = liste;
        }
        catch { }
