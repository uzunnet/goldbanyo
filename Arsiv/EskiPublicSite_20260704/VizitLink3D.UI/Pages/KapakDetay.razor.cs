...
public partial class KapakDetay : ComponentBase, IDisposable
{
    ...
    {
        // Gold Banyo frontend: kapı/kapak sayfaları kaldırıldı, ana sayfaya yönlendir
    ...
    private async Task ModelVerileriniYukleAsync()
    {
        ...
        if (model != null)
        {
            ...
            {
                var urun = await api.GetAsync<VizitLink3D.Ortak.Modeller.Urunler.Urun>($"api/urunler/slug/{model.Slug}");
                if (urun != null)
                {
            ...
            if (!string.IsNullOrWhiteSpace(closeup) && closeup != "/medya/vizitlink3d_default.png"
