...
public partial class Referanslar : ComponentBase, IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        _sayfaBasligi = dil.T("referanslar.sayfaBasligi", "Referanslar | Gold Banyo");
    ...
    private async Task VerileriYukleAsync()
    {
        ...
        try
        {
            var refListe = await api.GetAsync<List<RefOgesi>>("api/referanslar");
            _referanslar = refListe ?? [];

            var yorumListe = await api.GetAsync<List<YorumOgesi>>("api/musteri-yorumlari");
