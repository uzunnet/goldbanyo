...
public partial class KapakSistemleri : ComponentBase, IDisposable
{
    ...
    private string _sayfaBasligi = "Kapak Sistemleri | VizitLink3D";
    private string _sayfaAciklamasi = "Membran, lake, laminat, melamin ve kaplama kapak se�eneklerimizle mutfak ve banyolar�n�z i�in m�kemmel ��z�m� bulun.";
    private string _heroBaslik = "Kapak Sistemleri";
    private string _heroAltBaslik = "�R�N KATALO�U";
    private bool _yukleniyor = true;
    private string _aramaMetni = string.Empty;
    private string _secilenKategori = "T�m�";
    ...
    private List<string> _kategoriler = ["T�m�"];
    ...
    private int _toplamUrun => _tumKapaklar
        .Where(k => _secilenKategori == "T�m�" || k.Kategori == _secilenKategori)
    ...
    {
        // Gold Banyo frontend: kapak sayfaları kaldırıldı, ana sayfaya yönlendir
    ...
    private void SayfaTurunuBelirle()
    {
        ...
        {
            _sayfaBasligi = "Kap� Modelleri | VizitLink3D";
            _sayfaAciklamasi = "Lake, membran, caml�, d�z ve �zel seri kap� modellerimizi inceleyin.";
            _heroBaslik = "Kap� Modelleri";
            _heroAltBaslik = "D�NAM�K MODEL KATALO�U";
        }
    }
    ...
    private async Task SayfaIceriginiYukleAsync()
    {
        ...
        var sozluk = await api.GetAsync<Dictionary<string, string>>($"api/sayfa-icerigi/{sayfaKodu}?dil={dil.AktifDil}");
        if (sozluk != null)
        {
    ...
    {
        var sayfaKodu = _aktifModelTuru == "Kapi" ? "kapi-modelleri" : "kapak-sistemleri";
        var ayar = await api.GetAsync<SayfaDuzenAyariDto>($"api/sayfa-duzen-ayarlari/{sayfaKodu}");
        if (ayar != null)
        {
    ...
    {
        // E�er AnaGorselUrl bo�sa ModelKodu'ndan say�sal k�s�m alarak yatay_ fallback �ret
        if (string.IsNullOrWhiteSpace(model.AnaGorselUrl))
        {
            ...
            model.AnaGorselUrl = string.IsNullOrWhiteSpace(numStr)
                ...
                : "/medya/gold-katalog/sayfa-006-hero.png";
        }
        else
        {
